 ...
 * @version 0.1
 */
class SHMCache extends CacheAbstract
{
    /**
    ...
    {
    	if (isset(self::$ids[$this->key]))
    		return  self::$ids[$this->key];
    	
    	foreach (array_keys(config::$cache_conf) as $i => $key)
    	{
    		if($this->key == $key)
    			return self::$ids[$key] = $i;
    	}
